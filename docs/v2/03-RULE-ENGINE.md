...
- ✅ GitHub Activation Runbook toegevoegd (`docs/v2/ci/03-GITHUB-ACTIVATION-RUNBOOK.md`) met exacte terminalcommando's om de lokale repository naar GitHub te pushen en Atlas CI live te activeren, inclusief CI-docs index (`docs/v2/ci/README.md`); runbook bijgewerkt met SSH-aanbeveling, PAT workflow-scope vereiste, troubleshooting en public-repo vereiste voor branch protection; repository succesvol gepusht naar `https://github.com/ClubDisplay/calculatieloket`, test PR bevestigt groene `Atlas CI / Run Atlas CI checks` en branch protection voor `main` is actief (Sprint 075).
